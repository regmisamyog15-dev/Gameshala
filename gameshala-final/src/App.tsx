...
const PRICING = [
  { label: "Solo Session", price: "Rs. 80", unit: "/ hour", desc: "1 player — perfect for solo adventures or practice", color: "var(--neon-blue)", popular: false },
  { label: "Duo Session", price: "Rs. 150", unit: "/ hour", desc: "2 players — best value for VS battles with a friend", color: "var(--neon-purple)", popular: true },
  { label: "Pool Table", price: "Hourly", unit: "rate", desc: "Dedicated billiards area — ask at the counter for rates", color: "var(--neon-green)", popular: false },
  { label: "Tournament Entry", price: "Rs. 500", unit: "per team", desc: "Monthly esports tournaments with cash prize pools", color: "var(--neon-red)", popular: false },
];
...
const FAQS = [
  ...
  { q: "Do I need to book in advance?", a: "Walk-ins are welcome! For large groups or tournament participation, messaging us on Instagram beforehand is recommended to ensure availability." },
  { q: "What games are available on PS5?", a: "Our catalog includes EA Sports FC 26, NBA 2K, God of War, Call of Duty, PUBG, Cricket 24, UFC, WWE Wrestling, and various racing titles — all kept updated." },
  { q: "Can I buy or sell game discs here?", a: "Yes! We operate a physical counter for buying, selling, and repairing physical game discs. We also sell PlayStation Network (PSN) digital games." },
...
function Navbar() {
  ...
  const links = [["Games","#games"],["Pricing","#pricing"],["Tournaments","#tournaments"],["Discs","#discs"],["FAQ","#faq"],["Contact","#contact"]];
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-300"
      style={{ background: scrolled ? "rgba(4,4,10,0.97)" : "transparent", backdropFilter: scrolled ? "blur(16px)" : "none", borderBottom: scrolled ? "1px solid rgba(0,212,255,0.15)" : "none" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        <a href="#hero" className="flex items-center gap-2">
          <Gamepad2 size={22} style={{ color: "var(--neon-blue)" }} className="animate-rgb" />
          <span className="section-title gradient-text-blue" style={{ fontSize: "0.8rem" }}>GAMESHALA</span>
        </a>
        <div className="hidden md:flex items-center gap-6">
          {links.map(([label, href]) => (
            <a key={label} href={href} className="text-gray-400 hover:text-white">{label}</a>
          ))}
        </div>
        ...
        <button className="md:hidden text-gray-300 hover:text-white" onClick={() => setOpen(!open)} aria-label="Toggle menu">
          {open ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>
      {open && (
        <div className="md:hidden px-4 pb-6 pt-2 flex flex-col gap-4"
          style={{ background: "rgba(4,4,10,0.98)", borderBottom: "1px solid rgba(0,212,255,0.15)" }}>
          {links.map(([label, href]) => (
            <a key={label} href={href} className="text-gray-300 hover:text-white text-sm" onClick={() => setOpen(false)}>{label}</a>
          ))}
...
function Hero() {
  const isOpen = useIsOpen();
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center rgb-grid overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 60% at 50% 40%, rgba(0,212,255,0.06) 0%, transparent 70%), radial-gradient(ellipse 50% 50% at 80% 80%, rgba(180,0,255,0.05) 0%, transparent 60%)" }} />
      <div className="absolute left-0 right-0 h-px pointer-events-none" style={{ background: "linear-gradient(90deg, transparent, rgba(0,212,255,0.4), transparent)", animation: "scanLine 4s linear infinite" }} />
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
        <div className="flex justify-center mb-6">
          <span className={`status-open text-sm px-4 py-2 rounded-full ${!isOpen ? "status-closed" : ""}`}
            style={{ background: isOpen ? "rgba(0,255,136,0.08)" : "rgba(255,45,85,0.08)", border: `1px solid ${isOpen ? "rgba(0,255,136,0.3)" : "rgba(255,45,85,0.3)"}` }}>
            <span className={`status-dot ${!isOpen ? "status-dot-closed" : ""}`} />
            {isOpen ? "NOW OPEN" : "CURRENTLY CLOSED"} &nbsp;·&nbsp; {BUSINESS.hours.label}
          </span>
        </div>
        <h1 className="section-title mb-4" style={{ fontSize: "clamp(2rem, 6vw, 4.5rem)", lineHeight: 1.1 }}>
          <span className="gradient-text-blue">GAMESHALA</span><br />
          <span style={{ color: "#e2e8f0", fontSize: "0.55em", fontWeight: 400, letterSpacing: "0.3em" }}>GAMING LOUNGE</span>
        </h1>
        <p className="text-gray-400 mb-3 max-w-xl mx-auto" style={{ fontSize: "1.15rem", letterSpacing: "0.05em" }}>
          Pokhara's most affordable PS5 gaming hub — premium consoles, pool table, and monthly esports tournaments in the heart of Mahendrapool.
        </p>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-16">
          ...
          <a href={`tel:${BUSINESS.phone}`}>
            <button className="btn-neon btn-neon-purple text-sm px-8 py-3">Call Us</button>
          </a>
          <a href="#games">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto">
          {[
            ...
            <div key={sub} className="card-glow rounded-lg p-4 text-center" style={{ background: "var(--bg-card)" }}>
              <div className="flex justify-center mb-1" style={{ color }}>{icon}</div>
              <div className="section-title text-sm font-bold" style={{ color }}>{val}</div>
              <div className="text-gray-500 text-xs mt-1">{sub}</div>
            </div>
          ))}
        </div>
        <div className="flex justify-center mt-16 animate-float">
          <a href="#games"><ChevronDown size={28} style={{ color: "var(--neon-blue)", opacity: 0.6 }} /></a>
        </div>
      </div>
    </section>
  );
    ...
    <section id="games" className="py-24 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-14">
          ...
          <p className="text-gray-400 max-w-lg mx-auto">All titles are regularly updated. Ask at the counter for our full catalog.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-10">
          <div className="card-glow rounded-xl p-8 flex items-center gap-6" style={{ background: "var(--bg-card)", borderColor: "rgba(0,212,255,0.3)" }}>
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Latest PS5 consoles with premium DualSense controllers and 4K display. Comfortable professional seating for long sessions.</p>
            </div>
          </div>
          <div className="card-glow rounded-xl p-8 flex items-center gap-6" style={{ background: "var(--bg-card)", borderColor: "rgba(0,255,136,0.3)" }}>
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Dedicated billiards area billed at an affordable hourly rate. Great for a break between gaming sessions.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="pricing" className="py-24 px-4 sm:px-6 rgb-grid" style={{ background: "var(--bg-card2)" }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          <p className="text-gray-400 max-w-lg mx-auto">Budget-friendly gaming — no hidden fees. Confirm exact rates at the counter.</p>
        </div>
    ...
    <section id="tournaments" className="py-24 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 max-w-lg mx-auto">Compete for real cash prizes. Monthly competitive tournaments — currently featuring EA Sports FC 26.</p>
        </div>
...
function Discs() {
  return (
    <section id="discs" className="py-24 px-4 sm:px-6" style={{ background: "var(--bg-card2)" }}>
      <div className="max-w-4xl mx-auto text-center">
        <p className="text-xs section-title neon-blue tracking-widest mb-2">DIGITAL STOREFRONT</p>
        <h2 className="section-title text-3xl sm:text-4xl gradient-text-blue mb-4">Buy, Sell &amp; Repair Discs</h2>
        <p className="text-gray-400 max-w-xl mx-auto mb-12">Physical game disc counter — buy, sell, and repair PlayStation discs. PSN digital game codes also available.</p>
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={title} className="card-glow rounded-xl p-7 text-center" style={{ background: "var(--bg-dark)" }}>
              <div className="flex justify-center mb-4" style={{ color }}>{icon}</div>
...
function Contact() {
  const isOpen = useIsOpen();
  return (
    <section id="contact" className="py-24 px-4 sm:px-6 rgb-grid" style={{ background: "var(--bg-card2)" }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          <div className="card-glow rounded-2xl p-8 flex flex-col gap-6" style={{ background: "var(--bg-dark)" }}>
            ...
            <div className="flex gap-4">
              <a href={BUSINESS.instagram} target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: "rgba(180,0,255,0.1)", border: "1px solid rgba(180,0,255,0.3)", color: "var(--neon-purple)" }}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <a href={`tel:${BUSINESS.phone}`}>
                <button className="btn-neon btn-neon-purple w-full text-sm py-3">Call {BUSINESS.phoneFormatted}</button>
              </a>
            </div>
          </div>
          <div className="card-glow rounded-2xl overflow-hidden" style={{ minHeight: 380, background: "var(--bg-dark)" }}>
            <iframe
              title="Gameshala Gaming Lounge — Mahendrapool, Pokhara"
